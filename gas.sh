...
kernel_name="LOSS_MIDO"
zip_name="$kernel_name-$(date +"%d%m%Y-%H%M").zip"
TC_DIR=$HOME/tc/
CLANG_DIR=$TC_DIR/clang-r450784d
export CONFIG_FILE="msm8953_defconfig"
export ARCH="arm64"
...
if ! [ -d "$TC_DIR" ]; then
    ...
    if ! git clone -q --depth=1 --single-branch https://android.googlesource.com/platform/prebuilts/clang/host/linux-x86 -b android13-gsi $TC_DIR; then
